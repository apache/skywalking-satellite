...
const (
	satellite   = "satellite:11800"
	service     = "consumer"
	upstreamURL = "http://provider:8080/hello"
)

func main() {
	report, err := reporter.NewGRPCReporter(satellite)
	if err != nil {
	...
	route.HandleFunc("/hello", func(writer http.ResponseWriter, request *http.Request) {
		clientReq, err := http.NewRequest(http.MethodGet, upstreamURL, nil)
		if err != nil {
